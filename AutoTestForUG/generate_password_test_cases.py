...
"""
密码强度测试用例生成器
用于生成符合指定规则的密码测试用例，覆盖各种字符类型和长度组合
"""
...
def generate_account_password_pairs(start_account: int = 670000, end_account: int = 670100) -> List[Tuple[str, str, str]]:
    """
...
if __name__ == "__main__":
    ...
    print(f"\n共生成 {len(password_cases)} 个密码测试用例:")
    print("-" * 50)
    
    for i, (desc, pwd) in enumerate(password_cases[:20], 1):  # 只打印前20个作为示例
        print(f"{i:2d}. [{desc}] '{pwd}' (长度: {len(pwd)})")
    
    if len(password_cases) > 20:
        print(f"... 还有 {len(password_cases) - 20} 个测试用例")
    
    print("\n" + "=" * 80)
    print("账号-密码对生成 (示例前30个):")
    print("=" * 80)
    
    # 修正账号范围说明 - 用户可能是想表达从670000开始的100个账号
    account_password_pairs = generate_account_password_pairs(670000, 670029)  # 示例前30个账号
    
    for i, (acc, pwd, desc) in enumerate(account_password_pairs, 1):
        print(f"{i+669999:6d}. 账号: {acc}, 密码: '{pwd}', 类型: [{desc}] (长度: {len(pwd)})")
    
    print(f"\n总共生成了 {len(account_password_pairs)} 个账号-密码对测试用例")
    print("密码规则: 4-12位，仅包含数字、英文字母和英文标点符号")
    print("覆盖9种字符类型组合: 纯数字、纯小写字母、纯大写字母、大小写混合字母、")
    print("                   纯标点符号、数字+字母、数字+标点、字母+标点、三者混合")