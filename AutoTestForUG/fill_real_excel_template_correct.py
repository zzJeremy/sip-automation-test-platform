...
def fill_real_excel_template(start_account: int = 670100, end_account: int = 670799, template_file: str = "号码配置.xlsx", output_file: str = "号码配置.xlsx"):
    ...
    for i in range(total_accounts):
        ...
        if i % 2 == 0:
            # 偶数索引分配给FXS账号
            row_data['FXS帐号'] = account
        else:
            # 奇数索引分配给IP账号
            row_data['ip帐号'] = account
...
if __name__ == "__main__":
    # 在真实Excel模板中填写数据
    count = fill_real_excel_template(670100, 670799, "号码配置.xlsx", "号码配置.xlsx")
